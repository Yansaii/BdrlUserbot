...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"gangstar(?: |$)(.*)"))
async def _(event):
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"plane(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"dump(?: |$)(.*)"))
async def _(message):
    try:
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"kiss(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("😗😙😚😚😘"))
