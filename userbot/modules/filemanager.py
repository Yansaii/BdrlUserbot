...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"ls(?: |$)(.*)"))
async def lst(event):
    if event.fwd_from:
        return
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"rm(?: |$)(.*)"))
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"rn ([^|]+)\|([^|]+)"))
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"zip (.*)"))
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"unzip (.*)"))
