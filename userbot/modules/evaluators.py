...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"eval(?:\s|$)([\s\S]*)"))
async def _(event):
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"exec(?: |$|\n)([\s\S]*)"))
async def run(event):
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"term(?: |$|\n)([\s\S]*)"))
...
@bot.on(bdrl_cmd(outgoing=True, pattern=r"json$"))
async def _(event):
    if event.fwd_from:
        return
